...
*   **Role-Based Access Control (RBAC):** 
    *   **ADMIN**: Manages departments, asset categories, audit cycles, and employee/role assignment (Organization Setup). Views organization-wide analytics.
        *   *Backend:* `@PreAuthorize("hasRole('ADMIN')")` for `/api/v1/departments` (POST/PUT/DELETE), user management, and organization setup.
    *   **ASSET_MANAGER**: Registers and allocates assets. Approves transfers, maintenance requests, and audit discrepancy resolution. Approves asset returns and condition check-in notes.
        *   *Backend:* `@PreAuthorize("hasAnyRole('ADMIN', 'ASSET_MANAGER')")` for asset lifecycle operations, maintenance approvals, and audit resolution.
    *   **DEPARTMENT_HEAD**: Views assets allocated to their department. Approves allocation/transfer requests within their department. Books shared resources on behalf of the department.
        *   *Backend:* `@PreAuthorize("hasAnyRole('ADMIN', 'DEPARTMENT_HEAD')")` for department-level asset approvals and booking resources.
    *   **EMPLOYEE**: Views assets allocated to them. Books shared resources. Raises maintenance requests. Initiates return/transfer requests.
        *   *Backend:* Basic authenticated access (`@PreAuthorize("hasAnyRole('ADMIN', 'ASSET_MANAGER', 'DEPARTMENT_HEAD', 'EMPLOYEE')")`), but operations must be strictly scoped to their own `user_id` at the service layer.
