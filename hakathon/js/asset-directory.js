/* Asset Directory JS */
const AssetDir = (() => {
  let state = { page: 1, pageSize: 10, search: '', category: '', status: '', dept: '', sort: 'assetTag', dir: 'asc', selected: new Set(), assets: [], total: 0 };
  ...
  const load = async () => {
    ...
    // Client-side filter by category and dept (since backend may not support query params yet)
    let filtered = state.assets;
    if (state.category) filtered = filtered.filter(a => (a.category||'').toLowerCase() === state.category.toLowerCase());
    if (state.dept)     filtered = filtered.filter(a => (a.department||'').toLowerCase() === state.dept.toLowerCase());
    if (state.status)   filtered = filtered.filter(a => (a.status||'').toLowerCase() === state.status.toLowerCase());
    if (state.search)   filtered = filtered.filter(a =>
      (a.assetTag||'').toLowerCase().includes(state.search) ||
      (a.name||'').toLowerCase().includes(state.search) ||
      (a.serialNo||'').toLowerCase().includes(state.search));
    ...
    const pageData  = filtered.slice(pageStart, pageStart + state.pageSize);
    ...
    if (!pageData.length) {
      ...
      tbody.innerHTML = pageData.map(a => `
        <tr class="animate-fade-up">
          ...
          <td style="font-family:var(--font-mono);font-size:.875rem;font-weight:600"><a href="details.html?id=${a.id}" onclick="event.preventDefault(); window.location.href='details.html?id=${a.id}'">${Utils.escapeHtml(a.assetTag||'')}</a></td>
          <td><div style="font-weight:600">${Utils.escapeHtml(a.name||'')}</div></td>
          <td>
            <div style="font-size:.8125rem;color:var(--text-secondary)">${Utils.escapeHtml(a.category||'—')}</div>
            <div style="font-size:.75rem;color:var(--text-muted)">${Utils.escapeHtml(a.department||'')}</div>
          </td>
          <td><span class="badge badge-neutral">${Utils.escapeHtml(a.condition||a.assetCondition||'—')}</span></td>
          <td>
  ...
  const populateFilters = async () => {
    ...
    try {
      ...
      const deptList = Array.isArray(depts) ? depts : (depts.content || []);
      if (catSel)  catSel.innerHTML  = '<option value="">All Categories</option>'  + catList.map(c  => `<option value="${c.name}">${c.name}</option>`).join('');
      if (deptSel) deptSel.innerHTML = '<option value="">All Departments</option>' + deptList.map(d => `<option value="${d.name}">${d.name}</option>`).join('');
    } catch (_) {}
  };
...
const initAssetDirView = () => {
  ...
  // Map filter id → state key
  const filterKeyMap = { 'category-filter': 'category', 'status-filter': 'status', 'dept-filter': 'dept' };
  Object.entries(filterKeyMap).forEach(([id, stateKey]) => {
    const el = document.getElementById(id);
    if (el) {
      const newEl = el.cloneNode(true);
      el.parentNode.replaceChild(newEl, el);
      newEl.addEventListener('change', e => {
        AssetDir.state[stateKey] = e.target.value;
