      ...
      if(val.length >= 3) {
        try {
          ...
          if (assets.length > 0) {
            ...
            if (infoText) infoText.innerHTML = `Found: <strong>${Utils.escapeHtml(assets[0].assetTag || assets[0].tag || '')}</strong> — ${Utils.escapeHtml(assets[0].name)} <span class="badge badge-primary">${assets[0].status}</span> (Assigned to: <strong>${Utils.escapeHtml(assets[0].assignedTo || 'Unknown')}</strong>)`;
          } else {
