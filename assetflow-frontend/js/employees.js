...
const EmpPage = (() => {
  ...
  const loadDeptOptions = async (selectedVal = '') => {
    ...
    try {
      ...
      sel.innerHTML = '<option value="">Select Department…</option>' +
        list.map(d => `<option value="${d.name}" ${d.name === selectedVal ? 'selected' : ''}>${d.name}</option>`).join('');
    } catch {
  ...
  const edit = async (id) => {
    ...
    await loadDeptOptions(e.dept || e.department || '');
...
const initEmpView = async () => {
  ...
  // Populate department filter from DB
  const deptFilterSel = document.getElementById('dept-filter');
  if (deptFilterSel) {
    try {
      const depts = await API.departments.getAll();
      const list  = Array.isArray(depts) ? depts : (depts.content || []);
