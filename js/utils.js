...
const Utils = (() => {
  ...
  // ════════════════════════════════════════════════════════════
  // DYNAMIC SELECT / DROPDOWN POPULATION
  // ════════════════════════════════════════════════════════════
  // Fills a <select> with options built from an array of items.
  // - selectId:   id of the <select> element
  // - items:      array of objects (or the raw API response; if it
  //               has a `.content` array — a paginated Page<T> — that
  //               array is used automatically)
  // - valueKey:   property to use for option value (default 'id')
  // - labelKey:   property, or a function(item) => string, for the
  //               visible label
  // - placeholder:text for the first, empty option (default 'Select…')
  // - selected:   value to pre-select (used when editing a record)
  const populateSelect = (selectId, items, opts = {}) => {
    ...
    const list = Array.isArray(items) ? items : (items && items.content) || [];
