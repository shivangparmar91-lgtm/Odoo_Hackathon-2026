...
window.generateTag = () => {
  ...
  document.getElementById('asset-tag').value = `AF-${year}-${rnd}`;
...
const loadFormOptions = async () => {
  try {
    const categories = await API.categories.getAll('size=100');
    Utils.populateSelect('asset-category', categories, { placeholder: 'Select Category…' });
  } catch (err) { Utils.Toast.api(err); }

  try {
    const departments = await API.departments.getAll('size=100');
    Utils.populateSelect('asset-dept', departments, { placeholder: 'Select Department…' });
  } catch (err) { Utils.Toast.api(err); }
};
